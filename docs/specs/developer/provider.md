<!-- Last verified: 2026-08-26 by Claude Code (KC-06 / 0.13.0: NEW opt-in `MockProvider::with_repeat`/`repeating` cycle-flat-queue repeat mode; additive, `new`/`text`/`texts` keep one-shot default) -->
...
- **MockProvider repeat/cycle mode** [EXISTS] -- `MockProvider::with_repeat(responses, repeat)` / `MockProvider::repeating(responses)` (`src/provider/mock.rs`) cycle the response queue VERBATIM instead of consuming it destructively to the `"(no more mock responses)"` fallback, so one scripted session can re-emit its responses across many turns. `new`/`text`/`texts` keep `repeat` OFF (byte-behaviour-identical one-shot). The consumer owns cadence: supply a `[ToolCalls, Text]` repeating unit with a terminal text — a `[ToolCalls]`-only repeat is an unbounded tool loop bounded only by `max_turns` (the kernel cycles verbatim and never synthesizes a terminal response). Added in 0.13.0 for i-phi #121's kernel half.
