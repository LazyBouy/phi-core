<!-- Last verified: 2026-06-09 by Claude Code (KC-03 render-polish follow-up: rendered_label() is now the revert--prefixed form ([revert-lesson]/[revert-finding]/[revert-outcome]/[revert-checkpoint]) so the breadcrumb signals it came from revert_to_state; the on-node annotation appends after the original content on its OWN line (newline separator). Prior CC-19 revert steering dedup: the [continue_after_revert] steering text REFERENCES the tip node — tip.node_id.render() ([nN]) + newest.kind.rendered_label() + a SHORT head-elided gloss [label: …tail] from elide_breadcrumb_tail — instead of echoing the full breadcrumb. The kind→label match in weave_braking_annotations is extracted to the TagKind::rendered_label() helper (single source of truth). The on-node [label: text] annotation stays the full summary site; the pointer carries only a recognizable cue to the abandoned action (#74 / D-TEST-0069). #74 closes the CC-15→…→CC-19 braking-render arc.) -->
...
- When a revert landed on the trunk tip (the reverted-to node carries any revert
  ...
  directive:
  `[continue_after_revert] You just reverted to node n1. See [revert-lesson: …approach (write_file abandoned)] at n1. Continue forward: do the next uncompleted step; do NOT redo completed steps; do NOT stop.`
  The node-number is `tip.node_id.render()` (e.g. `n1`); the label is
  `newest.kind.rendered_label()` (`revert-lesson`/`revert-finding`/`revert-outcome`/`revert-checkpoint` — the
  single source of truth the weave shares; the `revert-` prefix signals the
  breadcrumb came from `revert_to_state`); the gloss is `elide_breadcrumb_tail`
...
- The reverted-to node also carries the **rewind breadcrumb** of the abandoned
  ...
  `[revert-lesson: …]` tag. This restores the model's progress thread after a rewind (a
