...
1. **Co-keep constraint specification.** `ToolCall` ↔ `ToolResult` are a bonded pair; the refresh filter must keep them atomically (or drop both). Without an explicit rule, dropped tool-call blocks orphan their results and the next LLM call fails at the provider layer. Other implicit pairs need cataloguing (assistant message + immediate tool-result follow-up). **KC-01 (#77 / D-TEST-0072) makes the revert/working-context render enforce this bonded-pair invariant CALL-ATOMICALLY (was node-atomic).** A parallel assistant node carrying N tool-calls (with N linearly-chained result nodes) is now drop/kept PER CALL keyed on the `(node_id, tool_call_id)` composite join: an assistant node emits only the calls whose result is live on the trunk (R1), an abandon-class collapse removes only the specific collapsing call/result and leaves live siblings intact (R2), a mixed-class node (abandon tag + live sibling) is kept whole — pin wins (R3), and a node left with zero calls + no text is dropped to avoid an empty-assistant 400 (R4). A declarative orphan-filter backstop (`enforce_call_atomic_backstop`) on the `streaming.rs` render pipeline is the locus-independent belt-and-suspenders. See `phi-core ADR-0001` + `src/types/context.rs::collapse_abandon_class_cluster`. The single-call (M=1) path is byte-identical. **KC-02 (#80 / D-TEST-0075) makes the PINNED (completion / step-summary) revert disposition TARGET-AWARE** (ADR-0002 §D2.1, SUPERSEDES ADR-0001 §D1.2's M=1 re-append + M=1 keep-whole): a pinned revert that lands onto the CALL node (sealing past the whole sub-task) RECLAIMS the abandoned parallel cluster uniformly into the durable breadcrumb (symmetric with abandon-class — `[DROP,…]`); a pinned revert that lands INTO the cluster (onto a RESULT node) KEEPS the WHOLE gathered cluster verbatim (every off-trunk sibling re-appended by call-node membership — `[PAIR,…]`), eliminating the silent keep-first/drop-rest loss; an on-trunk result is ALWAYS kept (a mid-trunk pinned cluster the model continued past stays verbatim). The 32-cell `category × revert-target × arity` matrix (+ 5 edge fixtures) in `collapse_abandon_class_cluster_tests` is the binding semantics; no provider 400 in any cell. Consequently KC-01's "M=1 byte-identical" no longer holds for the pinned-onto-call-node / into-cluster sub-cases (intended — full silent-loss-class closure). **KC-03 (#81 / D-TEST-0076) SUPERSEDES the KC-02 (B) result-node keep-whole with the SIMPLE TAIL-SHRINK CONTRACT** (ADR-0003 §D3.1, SUPERSEDES ADR-0002 §D2.1): a pinned revert onto X SHRINKS every node strictly after X uniformly across categories (Rule 1 — the result-node into-cluster case no longer re-appends the gathered tail); the pinned summary is ADDED to X *after* X's original content while X is KEPT (Rule 2/Rule 2a `original body → summary → [continue_after_revert]`), whereas abandon-class REPLACES X's content; the orphaned paired tool-call left on a kept node is surgically removed by id (Rule 3); the breadcrumb carries a SINGLE `[kind: …]` label naming ONLY the shrunk tools. The pinned arm thus CONVERGES to the abandon-class arm's already-correct shrink, differing only by Rule 2 (ADD vs REPLACE). The on-trunk-keep gate (a pinned cluster the model continued past, all results on-trunk, is kept verbatim) is PRESERVED as Rule 1's don't-over-shrink guard. KC-02's "result-node keeps the whole gathered cluster" no longer holds (intended — it was the misread #81 corrects).
